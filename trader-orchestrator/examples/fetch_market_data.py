...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Fetch market data from Interactive Brokers")
    parser.add_argument("symbol", nargs="?", default="AAPL", help="Symbol to fetch data for (default: AAPL)")
...
def main():
    ...
    try:
        ...
        # Create connector
        print(f"Creating connector for {config['ibkr']['host']}:{config['ibkr']['port']}")
        ...
        if bars.empty:
            ...
            print(bars.tail()[['date', 'open', 'high', 'low', 'close', 'sma50', 'green_candle']])
        ...
        if not chain:
            ...
            print(f"Retrieved option chain with {len(chain)} expirations and {total_options} total options")
            ...
            if not atm_options:
                ...
                for opt in atm_options:
                    print(f"  {opt.right} {opt.strike} {opt.lastTradeDateOrContractMonth}")
                
                # Get OTM options
                print(f"\nFinding OTM options for {symbol} (1 strike OTM)...")
                otm_calls = fetcher.get_atm_options(symbol, call_put="CALL", otm_offset=1)
                ...
                if atm_options:
                    ...
                    if greeks:
                        ...
                        print(f"  Bid/Ask: {greeks['bidPrice']:.2f}/{greeks['askPrice']:.2f}")
