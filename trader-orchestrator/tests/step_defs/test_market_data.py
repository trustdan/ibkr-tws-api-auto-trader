...
from ib_insync import IB, Stock, Option, BarData
...
def request_historical_bars(days, symbol, context):
    ...
    for i in range(days):
        ...
        close_price = 100 + i
        open_price = close_price - (1 if i % 3 == 0 else -1)  # Alternate green/red candles
...
def request_otm_call_options(symbol, offset, context):
    ...
    # Call the method
    context.options = context.fetcher.get_atm_options(symbol, call_put="CALL", otm_offset=offset)
