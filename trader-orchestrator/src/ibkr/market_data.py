...
import numpy as np
import pandas as pd
from ib_insync import IB, BarData, Contract, Option, Stock, util
...
class MarketDataFetcher:
            ...
            for expiration in chain.expirations:
                ...
                if len(strikes) > 20:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Could not get current price for {symbol}: {str(e)}"
                        )
    ...
    def get_atm_options(
        ...
        try:
            ...
            last_price = (
                ticker.last if hasattr(ticker, "last") and ticker.last else ticker.close
            )
            self.ib.cancelMktData(underlying)
            ...
            atm_idx = min(
                range(len(strikes)), key=lambda i: abs(strikes[i] - last_price)
            )
    ...
    def get_option_greeks(self, option: Option) -> dict:
        ...
        """
        logger.info(
            f"Fetching Greeks for {option.symbol} {option.right} {option.strike} {option.lastTradeDateOrContractMonth}"
        )
        ...
        try:
            ...
            greeks = {
                ...
                "optPrice": option_ticker.last
                if option_ticker.last
                else option_ticker.close,
