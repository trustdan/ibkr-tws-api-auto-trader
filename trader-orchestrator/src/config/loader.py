...
def _validate_config(config: Dict[str, Any]) -> None:
    ...
    ), "IBKR client_id must be an integer"

    # Strategy validation
    assert isinstance(
        config["strategy"]["sma_period"], int
    ), "SMA period must be an integer"
    assert config["strategy"]["sma_period"] > 0, "SMA period must be positive"

    assert isinstance(
        config["strategy"]["candle_count"], int
    ), "Candle count must be an integer"
    assert config["strategy"]["candle_count"] > 0, "Candle count must be positive"

    assert isinstance(
        config["strategy"]["otm_offset"], int
    ), "OTM offset must be an integer"
    assert config["strategy"]["otm_offset"] >= 0, "OTM offset must be non-negative"

    assert isinstance(
        config["strategy"]["iv_threshold"], float
    ), "IV threshold must be a float"
    assert (
        0 <= config["strategy"]["iv_threshold"] <= 1
    ), "IV threshold must be between 0 and 1"

    assert isinstance(
        config["strategy"]["min_reward_risk"], float
    ), "Min reward/risk must be a float"
